...
class TrafficCapture:
    ...
    def __init__(self, interface='lo', callback=None):
        # Auto-detect Windows loopback interface
        import platform
        if platform.system() == 'Windows' and interface == 'lo':
            ...
            if not loopback_found:
                # Use all interfaces as fallback
                self.interface = None
                print("Windows loopback not found, capturing on all interfaces")
        else:
    ...
    def start(self):
        ...
        def capture_loop():
            print(f"Starting traffic capture on interface: {self.interface if self.interface else 'ALL'}...")
            try:
                ...
                print("HINT: On Windows, make sure Npcap is installed with loopback support")
